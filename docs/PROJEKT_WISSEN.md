# 🧠 Klopatra – Projektwissen & Backlog
...
| 2026-07-31 | **Store-Pflicht: Käufe wiederherstellen + Datenschutz** | `restore()` in `purchases.js` (Web-Fallback, native via RevenueCat vorbereitet) + `restorePurchases`-Handler; „Käufe wiederherstellen"-Button im ProDialog **und** unter Einstellungen → Rechtliches (Apple Guideline 3.1.1, Pflicht). Neu: `public/datenschutz.html` (deutsche Datenschutzerklärung, lokal-only, mit Platzhaltern für Kontakt/Datum) → verlinkt in Einstellungen (`import.meta.env.BASE_URL`, funktioniert unter Pages-Subpfad) + im Disclaimer. Neuer Settings-Abschnitt „Rechtliches". |
| 2026-07-29 | **Store-Packaging vorbereitet (Capacitor)** | `@capacitor/*` (core/cli/ios/android) + `capacitor.config.json` (appId `com.robtechconsult.klopatra`, webDir `dist`). npm-Scripts `app:ios`/`app:android`/`app:sync`. Kauf-Abstraktion `purchases.js` (Produkt-IDs + Web-Fallback, native IAP via RevenueCat vorbereitet), verdrahtet in `buyPro`/`giveTip`. Native Projekte `ios/`/`android/` gitignored (lokal via `cap add`). Vollständige Anleitung: **STORE.md**. Web-Deploy unverändert. |
